      ...
      <div className="bg-gray-100 h-fit flex gap-2 flex-col justify-center items-center">
        ...
        {orderItems.map((element, index) => {
          return (
            <div
              key={index}
              onClick={() => handleCanteenClick(index)}
              className="h-fit w-fit flex flex-col justify-center items-center bg-gray-400 rounded-lg"
            >
              <img
                src={element.orderFood[0].foodImg}
                className="h-[5rem] w-[5rem] m-1 mt-3 rounded-md"
              ></img>
