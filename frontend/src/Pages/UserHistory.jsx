...
function UserHistory() {
  ...
  function handleClick(isMenu) {
    if (isMenu) return navigator("..");
    navigator("/user");
  }
        ...
        <div>
          ...
          <div className="bg-white dark:bg-[#181818] p-5 rounded-lg shadow-lg ">
            {totalOrders.currentOrders.length > 0 ? (
              totalOrders.currentOrders.map((orderGroup, ind) => (
                <div key={ind + 1}>
          ...
          <div className="bg-white dark:bg-[#181818] p-5 rounded-lg shadow-lg">
            {totalOrders.orderHistory.length > 0 ? (
              totalOrders.orderHistory.map((orderGroup) => (
                <div
                  key={orderGroup._id}
                  className="border-2 border-green-900 mb-2 rounded-md px-2"
                >
                  {orderGroup.orders.map((order) => (
                    <div
                      key={order._id}
                      className="flex items-center border-b border-green-200 dark:border-green-950 py-4"
                    >
                      <img
                        src={order.photo}
                        alt={order.itemName}
